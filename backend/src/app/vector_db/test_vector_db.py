...
def test_create_points_payload_structure():
    ...
    assert points[0].payload["language"] == "en"  # default fallback
...
def test_embedding_dimensions(embedder):
    """Ensure vectors conform to the 384-dimension configuration."""
...
def in_memory_qdrant():
    """
    Creates an isolated in-memory Qdrant instance without connecting
    to an external Qdrant cluster.
    """
...
def test_qdrant_upsert_and_search(in_memory_qdrant, embedder):
    ...
    docs = [
        {"text": "Farming subsidies and crop loans", "source": "agri_01", "section": "Page 1", "domain": "agriculture"},
        {"text": "Income tax rebate regulations", "source": "fin_01", "section": "Page 1", "domain": "finance"}
    ]
